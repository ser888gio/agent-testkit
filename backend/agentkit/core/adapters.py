...
from agentkit.core.schema import AssertionResult, Category, Risk, Status, TestResult
...
class ExternalEvalAdapter(ABC):
    """One third-party eval engine, expressed in agentkit terms."""
    ...
    def normalize(
        ...
        *,
        ...
        """Turn one report from the tool into redacted agentkit results."""


class PromptfooAdapter(ExternalEvalAdapter):
    """promptfoo as a declarative red-team backend behind the agentkit runtime."""
    ...
    def to_config(
        ...
        *,
        ...
        return {
            "description": f"agentkit harness for {profile.id}",
            "targets": [
                {
                    "id": "http",
                    "label": profile.id,
...
class GarakAdapter(ExternalEvalAdapter):
    ...
    def command(self, endpoint: str, probes: list[str], *, report_prefix: str) -> list[str]:
        """The argv agentkit would run. Returned rather than executed: the caller
