# `agentaudit/packs` — test content

These YAML files are **product data, not pytest tests**. They are the test cases agentaudit
runs against a customer's agent. `pytest` does not collect them;
`agentaudit.core.loader:discover` does.
