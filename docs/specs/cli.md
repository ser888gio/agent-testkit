...
**Task 16 · Depends on: 11,12,13 · Files:** `agentaudit/cli.py`, `tests/test_cli.py`

## Goal
The operator surface: run tests (with CI-gating exit codes), export reports, launch the UI.
Built with **Typer**; entrypoint `agentaudit = "agentaudit.cli:app"` (already in pyproject).

## Commands

### `agentaudit run`
```
agentaudit run <packs_dir> --target <config.yaml> [--db agentaudit.db]
...
Output (table):
```
agentaudit run — target: treasury-demo   (12 tests)
...
### `agentaudit report`
```
agentaudit report --run <run_id> --format json|html|junit|md [--out path] [--db …]
```
Delegates to `feat/reports`; writes to `--out` or stdout.

### `agentaudit ui`
```
agentaudit ui [--host 127.0.0.1] [--port 8000] [--db …]
```
Launches uvicorn on `agentaudit.web.app:app`; prints the URL.
Loopback startup selects explicit local development authentication when no mode is configured.
Non-loopback startup requires `AGENTAUDIT_AUTH_MODE=oidc` and complete OIDC settings.
...
## Done when
All three commands work from a clean `pip install -e .`; `agentaudit run` returns the correct
