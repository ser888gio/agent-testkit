...
Persist tenant-owned runs and scores to SQLite and provide the read helpers used by the CLI
and dashboard. `Store` is the repository's only application SQL boundary.
