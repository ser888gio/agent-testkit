...
1. **Correctness and regressions.** Does it do what it claims? Check every consumer of a
   changed signature, schema field, or contract — including YAML packs in `agentkit/packs/`
   and Jinja templates in `frontend/agentkit/web/templates/`, which reference names as
...
Also flag: renamed pack `id:` values (breaks `agentkit compare` history), new `Category` or
