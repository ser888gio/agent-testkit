...
PACKAGE_DIR = Path(agentaudit.__file__).resolve().parent
...
def get_db_path() -> str:
    return os.environ.get("AGENTAUDIT_DB", "database/agentaudit.db")
...
def get_store() -> Store:
    ...
    if _store is None or _store_path != path:
        # The path only changes under tests, which point AGENTAUDIT_DB at a tmp
...
def connect_agent_page(principal: Principal = Depends(current_principal)) -> HTMLResponse:
    ...
    # _resolve_within_allowed resolves these against CWD, so "agentaudit/config/x"
...
def settings_page(principal: Principal = Depends(current_principal)) -> HTMLResponse:
    auth_state = (
        f"OIDC ({os.environ['AGENTAUDIT_OIDC_ISSUER']})"
        if auth_enabled()
    ...
    return _render(
        ...
        settings={
            ...
            "egress_allow_local": os.environ.get("AGENTAUDIT_EGRESS_ALLOW_LOCAL") == "1",
        },
        principal=principal,
        active="settings",
    )
...
def get_artifacts_dir() -> Path:
    return Path(os.environ.get("AGENTAUDIT_ARTIFACTS_DIR", "database/artifacts"))
