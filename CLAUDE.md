...
**agentaudit** is a black-box testing kit for AI agents. It calls an agent through its endpoint
...
> `agentaudit` helps companies validate AI agents by generating a tailored black-box test
> harness that tries to break them, then turns the results into safety and compliance
> evidence.

Enterprise framing:

> `agentaudit` is a black-box testing and assurance platform for AI agents that discovers
...
`infra/alembic/` (migrations) ships inside the installed wheel as `agentaudit.migrations` /
`agentaudit.migrations.versions` — namespace-package `find` had no way to reach outside
`where`. At import time there is exactly one `agentaudit` package. `agentaudit.core` lives under
`backend/` (including `agentaudit.cli`), `agentaudit.web` under `frontend/`,
`agentaudit.packs`/`agentaudit.config` at the root, and `agentaudit.migrations` under
...
- **CLI** — `backend/agentaudit/cli.py` (Typer; the `agentaudit` console script)
- **Test content (data, not code)** — `agentaudit/packs/**/*.yaml`
...
- **DB schema migrations** — `infra/alembic/` (Alembic, raw SQL via `op.execute`, no ORM
  models); driven by the `agentaudit migrate` CLI subcommand, config in root `alembic.ini`

Generated / vendored — **never edit by hand:** `dist/`, `agentaudit.egg-info/`, `.venv/`,
`agentaudit.db`, `**/__pycache__/`, `uv.lock` (regenerate via `uv lock`),
...
| Apply DB migrations | `agentaudit migrate --db <path>` |
| Migration status | `agentaudit migrate --db <path> --status` |
...
- **`agentaudit` is configured as a first-party import** (`[tool.ruff.lint.isort]
...
`__pycache__` and was removed. The `agentaudit run ...` commands below are the maintained,
...
Product commands (exercising agentaudit itself, useful for manual verification):

```bash
agentaudit run agentaudit/packs/treasury --target agentaudit/config/treasury-agent.yaml
agentaudit run agentaudit/packs/agentic  --target agentaudit/config/treasury-agent.yaml --compliance
agentaudit plan agentaudit/packs/treasury --target agentaudit/config/treasury-agent.yaml
agentaudit run agentaudit/packs/treasury --target agentaudit/config/treasury-agent.yaml --plan --max-tests 8
agentaudit report --run <run_id> --format md      # also: json, junit, html, compliance, compliance-json, plan
agentaudit compare <run_id_a> <run_id_b>
agentaudit ui
```
...
5. **User-facing flows** — CLI change → `tests/test_cli.py`; dashboard change →
   `tests/test_web.py` plus a manual `agentaudit ui` check
...
- **Runner / control-plane split.** Runner modules (`core/agent.py`, `core/sandbox.py`,
  ...
  `backend/agentaudit/core/agent.py`. Keep it that way.
...
- **Test placement:** repo tests live in `tests/`, one file per module
  ...
  is data, not pytest.
- **Contracts:** the shared schema is `backend/agentaudit/core/schema.py`
...
- Keep `frontend/agentaudit/web/app.py` thin — push logic into `core/` where it is testable
...
Directory-level `CLAUDE.md` files exist under `backend/agentaudit/core/`,
`backend/agentaudit/domains/`, `frontend/agentaudit/web/`, `agentaudit/packs/`, and `tests/`;
